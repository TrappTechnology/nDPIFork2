...
static struct Root_data getRootDataStructure(const char* originalJsonStr)
{
    ...
    // comment out l4_proto as this is never hit
    //
...
static void add_Root_Data(json_object** root_object,  struct Root_data rootDataStructure, int flowRiskCount, char* proto_by_ip, char* protocol)
{
    ...
    //{
    //    json_object_object_add(network_object, "l4_proto", json_object_new_string(rootDataStructure.l4_proto));
    //    addNetwork = TRUE;
    //}

    if (rootDataStructure.proto != NULL)
    {
        json_object_object_add(network_object, "transport", json_object_new_string(rootDataStructure.proto));
        addNetwork = TRUE;
    }
...
void ConvertnDPIDataFormat(char* originalJsonStr, char** converted_json_str, size_t* createAlert, int flowRiskIndex, unsigned int response_status_code, char* user_agent, char* filename, char* content_type, char* request_content_type)
{
    ...
    *createAlert = ndpiData.flow_risk_count;
    ...
    {
        struct json_object* status_code_object = json_object_new_int(response_status_code);
        json_object_object_add(http_obj, "response.status_code", status_code_object);
        add = TRUE;
    }
